...
async function extractData() {
    ...
    const projects = await page.evaluate(() => {
        const projectsList = [];
        const projectsElements = document.querySelectorAll('[name="lp-single-project"]');
        
        projectsElements.forEach(projectElement => {
            const projectName = projectElement.querySelector('.css-13to1co img').alt.trim();
            const status = projectElement.querySelector('[data-bn-type="text"]').innerText.trim();
...
async function saveDataToFile(projects) {
    try {
        ...
        const newData = projects.filter(project => !existingProjects.some(existingProject => existingProject.projectName === project.projectName));
        ...
        if (newData.length > 0) {
            ...
            newData.forEach(project => {
                console.log(`1 novo projeto adicionado: ${project.projectName} - ${project.status}`);
